...
SELECT b.company, b.brand_name, f.year, f.revenue_usd_m,
       ROUND(100.0 * f.revenue_usd_m / SUM(f.revenue_usd_m) OVER (PARTITION BY f.year), 2) AS share_pct
...
SELECT b.molecule, b.brand_name, f.year, f.revenue_usd_m,
       ROUND(100.0 * (f.revenue_usd_m - LAG(f.revenue_usd_m) OVER (PARTITION BY f.brand_id ORDER BY f.year))
             / LAG(f.revenue_usd_m) OVER (PARTITION BY f.brand_id ORDER BY f.year), 1) AS yoy_growth_pct
FROM fact_brand_sales f
JOIN dim_brand b ON b.brand_id = f.brand_id
WHERE f.is_simulated = 0
ORDER BY b.molecule, f.year;

-- Q3 Launch Tracker：tirzepatide 上市后前 N 年销售曲线（首年=1）
WITH first_yr AS (
    SELECT brand_id, MIN(year) AS launch_year FROM fact_brand_sales GROUP BY brand_id
)
SELECT b.brand_name, f.year - fl.launch_year + 1 AS years_since_launch,
       f.revenue_usd_m
FROM fact_brand_sales f
JOIN first_yr fl ON fl.brand_id = f.brand_id
JOIN dim_brand b ON b.brand_id = f.brand_id
WHERE f.is_simulated = 0 AND b.company = 'Eli Lilly'
...
WHERE f2022.year = 2022 AND f2022.is_simulated = 0
ORDER BY fl.revenue_usd_m DESC;

-- Q5 真实世界安全信号：按分子的 Top 不良事件（RWE 监测）
SELECT molecule, event_type, SUM(report_count) AS total_reports,
...
SELECT company, phase, COUNT(*) AS trial_count, SUM(enrollment) AS total_enrollment
FROM fact_trials
WHERE is_simulated = 0
...
SELECT year, ROUND(revenue_usd_m / 1000.0, 2) AS market_usd_bn, is_simulated
